...
Multiple read files per sample are supported. Files can be listed explicitly with `;`, or local and S3 glob patterns can be used, or a combination of the two. For example, the following samplesheet combines two lanes of paired-end Illumina data per sample:

```csv title="samplesheet_multi_lane.csv"
sample_id,path,path_2,sequence_type
SAMPLE_1,lane1_R1.fastq.gz;lane2_R1.fastq.gz,lane1_R2.fastq.gz;lane2_R2.fastq.gz,illumina
SAMPLE_2,s3://my-bucket/sample2_lane*_R1.fastq.gz,s3://my-bucket/sample2_lane*_R2.fastq.gz,illumina
```
...
| `path`             | Path to input sequence data, typically gzipped FASTQ files. When paired end sequencing is used, this is used for the forward read's data and `path_2` is used for the reverse reads. This can be a local file path, an S3 path, or a URL to an online location. Multiple files of the same sequencing format can be supplied by separating them with `;`. Local and S3 glob patterns (e.g. `lane*_R1.fastq.gz`) are also supported. The `sequence_type` column must have a value. If a glob or semicolon-delimited list resolves to no files, the sample is skipped and a warning is added to the report. |
| `path_2`           | Path to the FASTQ files for the reverse read when paired-end sequencing is used. This can be a local file path, an S3 path, or a URL to an online location. Multiple reverse-read files can be supplied by separating them with `;`, and local/S3 glob patterns are supported. When multiple files are given in both `path` and `path_2`, the two columns must contain the same number of files. The `sequence_type` column must have a value.                                                                                                                                                            |
