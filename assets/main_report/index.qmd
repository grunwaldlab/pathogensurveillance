...
gene_count_data <- find_ps_data(params$inputs, target = 'gene_counts', simplify = TRUE, must_exist = FALSE)
has_core_genes <- !is.null(gene_count_data) && nrow(gene_count_data) > 0
n_core_genes <- if (has_core_genes) sum(gene_count_data$n_genes[gene_count_data$method == 'core'], na.rm = TRUE) else 0L
```
