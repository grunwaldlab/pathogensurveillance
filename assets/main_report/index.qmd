...
params:
    inputs: "/home/fosterz/projects/pathogensurveillance_publications/publication/analyses/identification_validation/output/manual_runs_output/serratia_bockelmannii_inputs"
execute:
    echo: false
...
```{r}
# TODO: replace with PathoSurveilR once updated
selected_ref_meta_table <- function(input, target, interactive = FALSE, ...) {
  ...
  # Check if downloaded reference metadata is available
  downloaded_ref_paths <- find_ps_paths(input, target = 'downloaded_ref_metadata', simplify = TRUE, must_exist = FALSE)
  if (!is.null(downloaded_ref_paths) && nrow(downloaded_ref_paths) > 0) {
  ...
  merged_data <- merged_data[, colnames(merged_data) %in% names(cols), drop = FALSE]
  colnames(merged_data) <- cols[colnames(merged_data)]
...
These are intended to be preliminary identifications used to select references for more robust analyses
...
```{r}
ani_matrix_path <- find_ps_paths(params$inputs, target = 'ani_matrix_csv', simplify = TRUE, must_exist = FALSE)
```

::: panel-tabset
### ANI top matches

```{r eval = !is.null(ani_matrix_path)}
estimated_ani_match_table(params$inputs, interactive = TRUE)
```

```{r, results='asis', eval = !is.null(ani_matrix_path)}
cat('
::: {.callout-tip collapse="true"}
## About this table
...
```{r, results='asis', eval = is.null(ani_matrix_path)}
cat('
ANI similarity data not available.
This can happen when no assemblies were successfully generated for any sample in this report group.
')
```

### ANI heatmap

```{r ani_heatmap, eval = !is.null(ani_matrix_path)}
estimated_ani_heatmap(params$inputs, interactive = TRUE, height = 1000, width = 950)
```
...
pocp_heatmap(params$inputs, interactive = TRUE, height = 1000, width = 950)
```
...
```{r}
multigene_tree_paths <- PathoSurveilR::find_ps_paths(params$inputs, target = 'multigene_tree', must_exist = F)
has_multigene_tree <- ! is.null(multigene_tree_paths)

gene_count_paths <- find_ps_paths(params$inputs, target = 'gene_counts', simplify = TRUE, must_exist = FALSE)

if (!is.null(gene_count_paths) && nrow(gene_count_paths) > 0) {
  gene_count_data <- find_ps_data(params$inputs, target='gene_counts', simplify=TRUE)
  n_core_genes <- sum(gene_count_data$n_genes[gene_count_data$method=='core'], na.rm=TRUE)
} else {
  gene_count_data <- NULL; n_core_genes <- 0L
}
```
...
This is a multigene phylogeny of samples with reference genomes for context, inferred from %s core gene(s). It is the most robust identification provided by this pipeline, but taxonomic coverage is still limited by the availability of similar reference sequences.
:::
', n_core_genes))
```
...
```{r, eval = has_multigene_tree}
selected_ref_meta_table(params$inputs, target = 'contextual_refs', interactive = TRUE)
```
...
```{r, eval = ! is.null(snp_tree_path_data)}
selected_ref_meta_table(params$inputs, target = 'mapping_refs', interactive = TRUE)
```

```{r, results='asis', eval = is.null(snp_tree_path_data)}
cat('
Could not generate SNP tree.
')
```
...
The **`nf-core/pathogen surveillance` pipeline** was developed by: Zachary S.L. Foster, Claire Worthy, Martha Sudermann, Nicholas C. Cauldron, Fernanda I. Bocardo, Hung Phan, [Alexandra Weisberg](https://alexandra-weisberg.com/), [Jeff Chang](http://changlab.cgrb.oregonstate.edu/) and [Nik Grünwald](http://grunwaldlab.cgrb.oregonstate.edu/).
