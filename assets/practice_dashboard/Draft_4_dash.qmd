...
nav-buttons:
 - icon: github
   href: https://github.com/grunwaldlab/pathogensurveillance 
 - icon: filetype-html
   href: https://grunwaldlab.github.io/pathogensurveillance_documentation/
nocite: |
...
```{r ani_heatmap .flow}
reference_data$ref_name <- reference_data$ref_id
...
```{r Practice_map2}
sample_data <- read_csv("updated_sample_data.csv")

pal <- colorFactor(
  palette = 'Dark2',
  domain = sample_data$year
)

m <- leaflet(data = sample_data) %>%
  addTiles() %>%
  addCircles(~longitude, ~latitude, color = ~pal(year))
m
```
:::
...
::: {.card title="Grunwald Lab" style="border:hidden"}
